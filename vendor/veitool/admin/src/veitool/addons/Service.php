...
class Service
{
     ...
     * @return array
     */
    public static function local(string $file, bool $force = false, array $extend = [])
    {
        ...
        if ($file_size > 10 * 1024 * 1024){
            throw new Exception('上传的文件大小不能超过10Mb');
        }
        //文件类型限制
        if ($file_ext != 'zip'){
            throw new Exception('请上传格式为zip的压缩包');
        }
        //获取插件临时目录
        ...
        try{
            ...
            $check = genv('app_debug', true) && config('veitool.unknown');
     ...
     * @return string
     */
    public static function getAddonClass(string $name, string $type = '', string|null $class = null)
    {
        ...
        $namespace = "\\plugin\\" . $name . ($type ? "\\$type\\" : "\\") . $class;
