...
class Dict extends AdminBase
{
    ...
    public function add()
    {
        ...
        if($d['sql']!=='' && !genv('app_debug', false)) return $this->returnMsg("非开发模式下禁止设置SQL查询语句，如需设置请配置.env文件");
    ...
    public function edit(string $do = '')
    {
        ...
        if($do=='up'){
            ...
            if($field=='title' && $value==''){
                ...
                }elseif(DG::one([['code', '=', $value],['id', '<>', $id]])){
            ...
            if($d['groupid'] == 1) return $this->returnMsg("所属类型不能为顶级类型");
            if($d['sql']!==$Myobj['sql'] && !genv('app_debug', false)) return $this->returnMsg("非开发模式下禁止修改SQL查询语句，如需修改请配置.env文件");
            if(DG::one([['code', '=', $d['code']],['id', '<>', $id]])) return $this->returnMsg("字典编码【{$d['code']}】已经存在");
