...
abstract class AdminBase extends BaseController
{
    ...
    protected final function __auth()
    {
        ...
        $this->routeUri = strtolower(str_replace(['app\\admin\\controller\\','plugin\\','app\\controller\\','\\'],['','','','/'],$this->request->controller)."/".$this->request->action.(($action = $this->request->get('action')) ? '/'.$action : ''));
        //验证权限
        $this->isPower();
    }
    ...
    {
        if(!$this->manUser = session(VT_MANAGER)){
            $url = VT_MAP.'/login/index';
            if($this->request->isAjax()){
                $this->exitMsg('您还未登录或已过期，请先登录！',401,['url'=>$url]);
            }else{
                throw new \app\exception\Redirect($url);
            }
        }
    }
    ...
    protected function logon(string $tip = '')
    {
        ...
        if(vconfig('admin_log',0)){
            \app\model\system\SystemManagerLog::add(['url'=>$this->routeUri.($tip ? ' '.$tip : ''),'username'=>$this->manUser['username'],'ip'=>$this->request->getRealIp()]);
        }
