...
abstract class BaseController
{
    ...
    public function __construct()
    {
        ...
        // 前台集中业务 获取应用名 app('http')->getName() 获取IP $this->request->getRealIp()
        $this->__home();
        // 验证（登录、权限）
        $this->__auth();
        // 控制器初始化
     ...
     */
    protected final function fetch(?string $tmp = null, string $tip = '', bool $logon = true)
    {
    ...
    {
        $tpl = $tpl ?: (is_mobile() ? 'err' : ROOT_PATH . 'app/v_msg.tpl');
    ...
    protected final function only($name = [], $type = 'post', $filter = 'strip_sql', $bin = true)
    {
        if(isset($name['@token'])){
            if(!genv('APP_DEBUG')){
                ...
                checkToken($arr[0],$arr[1]) === false && $this->exitMsg("Token错误");
        ...
        $item = [];
        $data = $bin ? $this->request->$type() : $this->request->$type('','',$filter);
        $preg = [
