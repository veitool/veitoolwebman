...
/**
 * 获取站点配置
 * @param  array|string   $name      配置键【支持:域1.域2，插件配置获取:@插件名.键名】 为数组时表示更新为当前数组信息【常驻内存时便于更新配置】
 * @param  string         $default   缺省值
 * @return array|string
 */
function vconfig($name='',$default=''){
    static $_VCF = [];
    $_VCF = is_array($name) ? $name : ($_VCF ?: \app\model\system\SystemSetting::cache());
 ...
 */
function is_mobile(){
    return preg_match('/(iphone|ipod|ipad|android|blackberry|webos|windows phone|mobile)/i', request()->header('user-agent'));
}
