...
class SystemOnline extends Base
{
    ...
    public function listQuery(array $where = [], array|string $order = ['last_time'=>'desc'], string $fields = '*', int $limit = 0)
    {
        ...
        if($page==1){
            ...
            //$this->where('last_time', '<', time() - 300)->delete();
        }
    ...
    public static function recod(array $user, string $url = '', int $type = 1)
    {
        ...
        self::create(['uid'=>$Online['uid'],'userid'=>$Online['userid'],'username'=>$Online['username'],'url'=>$url,'last_time'=>time(),'ip'=>request()->getRealIp(),'type'=>$type],['uid','userid','username','url','last_time','ip','type'],true);
    }
