...
class SystemOnline extends Base
{
    ...
    public static function recod(array $user, string $url = '', int $type = 1)
    {
        ...
        self::create(['uid'=>$Online['uid'],'userid'=>$Online['userid'],'username'=>$Online['username'],'url'=>$url,'etime'=>time(),'ip'=>request()->getRealIp(),'type'=>$type],['uid','userid','username','url','etime','ip','type'],true);
    }
