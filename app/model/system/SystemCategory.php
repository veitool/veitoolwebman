...
class SystemCategory extends Base
{
     ...
     * @return array
     */
    public static function cadd(string $type = '', array $d = [])
    {
        if(!$d['title']) return ['msg'=>'请输入类别名称'];
        $d['creator'] = $d['creator'] ?? '';
        $d['type'] = $type;
        $d['listorder'] = (int)$d['listorder'];
     ...
     * @return array
     */
    public static function cedit(string $do = '', string $type = '', array $d = [])
    {
        $catid = (int)$d['catid'];
        ...
        if($do=='up'){
            ...
            if($Myobj->save([$field=>$value,'editor'=>$d['editor'] ?? ''])){
     ...
     * 类别删除
     * @param  string         $type    标识
     * @param  string|array   $catid   类ID
     * @return array
     */
    public static function cdel(string $type = '', string|array $catid = [])
    {
